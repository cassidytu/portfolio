...
exports.del = exports.put = exports.post = exports.get = exports.getPackageMetadata = exports.getIdentUrl = exports.customPackageError = exports.handleInvalidAuthenticationError = exports.AuthType = void 0;
...
async function loadPackageMetadataInfoFromDisk(identPath) {
    return await core_1.miscUtils.getFactoryWithDefault(PACKAGE_DISK_METADATA_CACHE, identPath, async () => {
        let cached = null;
        try {
            cached = await fslib_1.xfs.readJsonPromise(identPath);
        }
        catch { }
        return cached;
    });
        ...
        return await get(getIdentUrl(ident), {
            ...
            wrapNetworkRequest: async (executor) => async () => {
                ...
                Promise.resolve().then(async () => {
                    ...
                    await fslib_1.xfs.mkdirPromise(fslib_1.ppath.dirname(identPathTemp), { recursive: true });
                    await fslib_1.xfs.writeJsonPromise(identPathTemp, metadata, { compact: true });
                    // Doing a rename is important to ensure the cache is atomic
                    await fslib_1.xfs.renamePromise(identPathTemp, identPath);
                }).catch(() => {
...
async function getPackageMetadata(ident, { cache, project, registry, headers, version, ...rest }) {
    ...
    const identPath = fslib_1.ppath.join(registryFolder, `${core_1.structUtils.slugifyIdent(ident)}.json`);
    let cached = null;
            ...
            if (configuration.get(`enableOfflineMode`)) {
                ...
                if (cache) {
                    for (const version of Object.keys(copy.versions)) {
                        const locator = core_1.structUtils.makeLocator(ident, `npm:${version}`);
                        const mirrorPath = cache.getLocatorMirrorPath(locator);
                        if (!mirrorPath || !fslib_1.xfs.existsSync(mirrorPath)) {
 ...
 */
const CACHE_KEY = core_1.hashUtils.makeHash(...CACHED_FIELDS).slice(0, 6);
function getRegistryFolder(configuration, registry) {
    const metadataFolder = getMetadataFolder(configuration);
    const parsed = new URL(registry);
    return fslib_1.ppath.join(metadataFolder, CACHE_KEY, parsed.hostname);
}
function getMetadataFolder(configuration) {
    return fslib_1.ppath.join(configuration.get(`globalFolder`), `metadata/npm`);
}
